...
router.post("/simulate", async (req, res) => {
  try {
    ...
    const { inputTokens, outputTokens, totalTokens } =
      simulateTokenCount(prompt);

    // Kostenberechnung pro 1.000.000 Tokens
    const cost =
      (inputTokens / 1_000_000) * Number(modelData.input_price_per_million) +
      (outputTokens / 1_000_000) * Number(modelData.output_price_per_million);
